...
public class ChestHealFullHP : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (pressed)
        {
            ...
            // Ocultamos el texto porque ya se abre.
            UpdatePromptVisibility();

            // Disparamos la animación de abrir si hay animator.
            if (anim)
                anim.SetTrigger("Open");

            // Curamos al jugador a tope.
            player.HealToFull();

            // Mostrar mensaje en la UI (si hay GameUI en escena).
